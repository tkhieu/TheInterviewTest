...
- **Three integration tests (T3.1)** — vitest + supertest. The refactor that paid for itself: extracted `buildApp()` from `index.ts` so tests bind the Express instance directly without `app.listen()` + a real port (no flake, no port collision). `fileParallelism: false` in `vitest.config.ts` because the three files share one Postgres database — without sequential file execution, `truncateAll` in one file's `beforeEach` would race against another file's data setup. **Test 3 specifically asserts `typeof stats.total === 'number'`**, which is the *correct* bigint guard: JSON serializes Postgres `bigint` as a string, and only `typeof === 'number'` differentiates from `Number.isFinite(parseInt(...))` (which would silently pass on the broken case). All 11 tests pass in 4.83s end-to-end against `docker compose up db`.
- **FE scaffold + MSW skeleton (T1.0)** — `packages/web` bootstrapped with Vite + React 18 + TS + Redux Toolkit + RTK Query + MSW v2. Three early choices that lock in the FE architecture: (a) **Vite proxy** on `/auth`/`/campaigns`/`/recipients` to `:4000` so dev requests are same-origin (no CORS) and MSW—if enabled—takes precedence by intercepting before the request leaves the browser; (b) `tailwind.config.cjs` extends `@campaign-manager/ui/tailwind.preset.cjs` *and* scans `../ui/src/**/*.{ts,tsx}` so the UI lib's class names survive Tailwind's purge (skipping that scan would render every imported component unstyled); (c) MSW v2 (`http.get` / `HttpResponse`), not v1 — the work-plan §7 example uses v1 syntax which is now legacy; implementing v2 from the start avoids a deprecation rewrite. Production build: 32 modules, 143 KB JS / 22 KB CSS (gzipped 46 KB / 4.8 KB).
- **Login + Campaign List (T1.5)** — full auth + list flow against MSW or real BE. **The `transformResponse` adapter (`adaptCampaignRow` in `src/api.ts`)** is the load-bearing piece — single function that maps BE wire (`recipients_count`/`send_rate`/`created_at`) to the UI lib's `Campaign` shape (`recipients`/`sentRate`/`created` as a relative-time string), per CLAUDE.md hard rule #1's "adapt in one place, never per-component" mandate. Same adapter survives whether the source is MSW or real BE — that property is what T1.6 will assert as a contract-conformance test. Token kept in Redux *memory only* per hard rule #4 (no localStorage); `RequireAuth` route guard reads from the slice and redirects to `/login` on no-token. `prepareHeaders` casts `getState() as {auth: {token}}` instead of importing `RootState` to avoid the circular `store.ts ↔ api.ts` import (RTK Query's documented workaround). Production build: 1657 modules, 305 KB JS / 23 KB CSS (gzipped 99 KB / 4.9 KB).
- **New Campaign page (T2.3)** — Input + Textarea + RecipientTagInput from the UI lib; client-side validation mirrors §6 caps (200 / 300 / 20,000 chars; 1–50 recipients with email regex). On submit, RTK Query's `createCampaign.invalidatesTags: ['Campaigns']` auto-refetches the list when the user navigates back — so "I just created X, where is it?" can't happen. The 50-recipient cap matters: `RecipientTagInput` itself enforces no max (it's a generic UI component), so paste-bombing 500 emails would create a "valid" UI state that the BE then 400s on submit; the inline cap turns "submit fails" into "field shows an error before submit." API errors flow through the §6 `error.data.error.message` envelope into the same `<p role="alert">` as field errors. The CampaignTable list page now also has a New campaign action via `<ActionButton asChild><Link>...</Link></ActionButton>` (the Slot pattern preserves button styling on the anchor element).
- **Detail page with live polling (T2.4)** — closes the user-visible loop: `<StatusBadge>` for all 5 statuses, conditional `<ActionButton>` cluster (Send when `draft|scheduled`, Delete when `draft`), `<StatCard>×4` (Recipients/Sent/Failed/Opened), `<ProgressBar>×2` (Send rate / Open rate with safe-divide-by-zero helpers), `<Accordion>` for body + recipients list. **The polling pattern is the most-instructive bit**: a *second* `useGetCampaignDetailQuery(id, {pollingInterval: detail?.status === 'sending' ? 3_000 : 0, skip: !detail})` call right after the initial query — RTK Query dedupes identical hook calls into one cache entry, so this is one network request, not two. Reading `detail?.status` from the *first* hook's data + setting `pollingInterval` dynamically + `skip` until first response = polling auto-starts on first `'sending'` response and auto-stops on `'sent'`/`'failed'`. Zero `useEffect`, zero manual `setInterval`, zero cleanup glue. RTK Query tag tagging refined to per-id (`{type: 'Campaigns', id}` for detail, `{type: 'Campaigns', id: 'LIST'}` for list) so deleting one campaign invalidates only its own cache entry + the list, not every cached detail page in memory. Production build: 1659 modules, 318 KB JS / 24 KB CSS (gzipped 103 KB / 5.1 KB) — the +24 KB CSS jump vs T2.3 reflects StatCard/ProgressBar/Accordion classes now actually surviving Tailwind's purge because the page renders them.
- **Adapter contract-conformance test (T1.6)** — vitest test that runs *both* the MSW handler data and a captured real-BE fixture (`src/mocks/__fixtures__/real-be-list.json`) through the *same* `adaptCampaignRow` and asserts identical UiCampaign output shape. Three properties make this a useful guard, not just a "the code runs" test: (a) shape-not-values (`typeof out.recipients === 'number'` etc.), since the `created` relative-time string is non-deterministic across runs; (b) **same key set assertion** — `Object.keys(adapt(msw)).sort() === Object.keys(adapt(realBe)).sort()` catches the "BE added a field but MSW didn't" drift before reviewers see it; (c) **explicit no-leakage check** — asserts `recipients_count`/`send_rate`/`created_at` do *not* appear on the adapter output, which catches a typo'd rename (e.g., spreading the input row instead of cherry-picking). The fixture-capture workflow is documented at the top of the test file as a `curl … | jq > __fixtures__/real-be-list.json` one-liner; per work-plan §9 DoD item 7, any PR that changes a §6 response shape must regenerate it in the same PR. 5 tests pass in 6ms (pure-CPU; no MSW server, no DB, no I/O).
- **FE polish (T3.2)** — four polish artifacts: (1) `Skeleton` is a 4-line component (`animate-pulse bg-surface-2`) consumed by list + detail loading states, replacing the placeholder "Loading…" text — gives reviewers a layout-shaped silhouette before data arrives; (2) `ErrorBoundary` is a class component (`getDerivedStateFromError` + `componentDidCatch`) wrapping the *entire* React tree at root in `main.tsx`, *not* per-page — a page-level boundary leaves the navigation chrome alive but useless when the page itself crashes, while root-level catches everything with a "Try again" reset; (3) sonner `<Toaster richColors closeButton />` for side-effect feedback only (Send started, Delete succeeded, errors) — *not* on idempotent reads, which would create noise; the §6 error envelope (`error.data.error.message`) feeds straight into the toast text via a small `extractErrorMessage` helper. (4) Avatar dropdown via Radix DropdownMenu — TopBar's `user` prop is just for display and exposes no `onAvatarClick`, so rather than expanding the UI lib API (CLAUDE.md forbids without test+story update), I dropped the prop entirely and rendered my own `<Avatar>` + `<DropdownMenu>` in `AppShell` next to the TopBar. Sign out dispatches `loggedOut()` then `navigate('/login', {replace: true})`; the new bundle weighs +106 KB (sonner ~10 KB + Radix DropdownMenu + its `react-presence`/`primitive`/`menu` deps ~90 KB) — acceptable for a real dropdown UX over a hand-rolled hover menu.

### What I plan to delegate

- **Seed script (T3.3)** — `demo@example.com` + 5 campaigns covering all 5 statuses + ~20 recipients per AC-R3
...
- **Docker DNS race on `compose up`** — first run failed with `getaddrinfo EAI_AGAIN db` from inside the api container, even though `depends_on.condition: service_healthy` was meant to gate startup. The race is real but subtle: `pg_isready` returning healthy means *Postgres is accepting self-connections*, not that the `db` hostname has finished propagating to the api container's network namespace. Fixed three layers deep: (a) `authenticateWithRetry()` in `src/index.ts` (10× 1s backoff — works regardless of compose timing, also helps with prod Postgres restarts), (b) `start_period: 10s` on the db healthcheck so the first health check waits for genuine readiness, (c) `restart: on-failure:3` on the api service. The retry on its own turned out to be insufficient on WSL2 — second report came in with **all 5 retries failing the same way**, which means DNS wasn't slow-to-register, it was *broken* (WSL2's interaction with Docker's embedded resolver at `127.0.0.11` is famously flaky after the WSL2 VM cycles its networking). Final fix: `links: [db]` on the api service, which injects an `/etc/hosts` entry for `db → <container IP>`. Linux nsswitch defaults to `hosts: files dns` so `/etc/hosts` wins and the embedded resolver is never queried. `links` is officially "deprecated" in Compose v2, but the implementation is alive and is the canonical escape hatch for exactly this case. Verified clean boot with `POSTGRES_PORT=5433 docker compose up -d --build` → `curl localhost:4000/health` → `{"ok":true}`, zero retry log lines.
