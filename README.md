...
- **Schedule + async send (T2.1)** and **Recipients endpoints (T2.2)** — atomic `UPDATE … WHERE status IN (...)` transition guards, idempotent `setImmediate` send loop with 90/10 random outcome per recipient
