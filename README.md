...
- **Docker DNS race on `compose up`** — first run failed with `getaddrinfo EAI_AGAIN db` from inside the api container, even though `depends_on.condition: service_healthy` was meant to gate startup. The race is real but subtle: `pg_isready` returning healthy means *Postgres is accepting self-connections*, not that the `db` hostname has finished propagating to the api container's network namespace. Fixed three layers deep so it can't recur: (a) `authenticateWithRetry()` in `src/index.ts` (10× 1s backoff — works regardless of compose timing, also helps with prod Postgres restarts), (b) `start_period: 10s` on the db healthcheck so the first health check waits for genuine readiness, (c) `restart: on-failure:3` on the api service as belt-and-suspenders. Verified end-to-end with `POSTGRES_PORT=5433 docker compose up -d --build` → `curl localhost:4000/health` → `{"ok":true}`.
